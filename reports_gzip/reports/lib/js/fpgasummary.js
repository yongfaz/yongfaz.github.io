...
function getAttributeClockParameter() {
  let mutliple = 0;  // assume the same
  let attributeClockValue = null;
  Object.keys(fmax_iiJSON['basicblocks']).forEach(function (blockName) {
    if (mutliple === 1) return;  // Done searching
    let block = fmax_iiJSON['basicblocks'][blockName];
    if (!isNaN(block['target_fmax'])) {
      // user specified something by attribute
      if (attributeClockValue === null) {
        attributeClockValue = parseInt(block['target_fmax']);
        return;
      }
      else {
        if (parseInt(block['target_fmax']) !== attributeClockValue) {
          mutliple = 1;
          return;
        }
      }
  ...
  if (mutliple) return -1;
  return attributeClockValue;
}
function getTargetFrequencyValue() {
  let defaultClockFreq = (infoJSON['compileInfo']['nodes'][0]['family'].indexOf('Stratix10') !== -1) ? 480 : 240;
  let compileClockValue = getCompileClockParameter();
  // return "Various" check fmax_ii to see if per component is used
  // workaround: parse fmax_ii find minimum achieved_fmax for each block
  let attributeClockValue = getAttributeClockParameter();
  if (compileClockValue === null && attributeClockValue === null) return defaultClockFreq;  // nothing specified
  if (attributeClockValue === null) return compileClockValue;  // only compile flag
  if (attributeClockValue === -1) return 'Various';  // too many attribute specification
  if (compileClockValue === null) return attributeClockValue;  // return single attribute level
  if (compileClockValue !== attributeClockValue) return 'Various';  // compile flag !== attribute
  return compileClockValue;  // return compile flag as it's same as attribute
}
    ...
    if (hasMapSummary || hasPerformanceSummary) {  // Have at least one thing
      ...
      if (mapSummary) {
        mapSummary['children'].forEach(function(child) {
          ...
          let attributeClockValue = null;
          Object.keys(fmax_iiJSON['basicblocks']).filter(function(blockName) {
            return (blockName.indexOf(shortName + '.') >= 0);
          }).forEach(function (blockName) {
            let block = fmax_iiJSON['basicblocks'][blockName];
            if (attributeClockValue === null || isNaN(attributeClockValue))
              attributeClockValue = block['target_fmax'];
          });
  ...
  this.draw = function(pShow) {
    ...
    if (summaryJSON.hasOwnProperty(this.vID) && summaryJSON[this.vID].hasOwnProperty('children') &&
      ...
      tmpSummary['children'].forEach(function (row) {
        ...
        let attributeClockValue = null;
        Object.keys(fmax_iiJSON['basicblocks']).filter(function(blockName) {
          return (blockName.indexOf(row['name'] + '.') >= 0);
        }).forEach(function (blockName) {
          let block = fmax_iiJSON['basicblocks'][blockName];
          if (attributeClockValue === null || isNaN(attributeClockValue))
            attributeClockValue = block['target_fmax'];
...
var FPGAClockFreqSummary = function(pDiv, pProduct) {
  ...
  this.draw = function(pShow) {
    ...
    if (!$.isEmptyObject(summaryJSON) && summaryJSON.hasOwnProperty('performanceSummary') && summaryJSON['performanceSummary'].hasOwnProperty('children') &&
        Array.isArray(summaryJSON['performanceSummary']['children']) && summaryJSON['performanceSummary']['children'].length !== 0) {
      ...
      Object.keys(fmax_iiJSON['basicblocks']).forEach(function (blockName) {
        let block = fmax_iiJSON['basicblocks'][blockName];
        if (block['achieved_fmax'] < minClock) {
          minClock = block['achieved_fmax'];
          minBlockName = block['name'];
        }
      });
      if (minClock < targetClock) {
        noteCall = 'changeDivContent(0,"See ' + minBlockName + ' in f<sub>MAX</sub> II Report for details.")';
