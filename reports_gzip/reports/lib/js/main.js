...
var MENU = {
    ...
    THROUGHPUT:    { value: 2, name: "Throughput Analysis", submenu: ["LOOP", "divider", "FMAX_II", "VERIF"] },
...
// Main menu properties
var CTRLS = {
  TREE:    { name: '&#xe056;', id: 'collapse_tree', collapsed: false, callback: collapseTree },    // "Show/Hide tree"
...
function buildFPGAReportMainMenu() {
  ...
  function generateTablesHTML() {
    ...
    // Preprocess data for new loops report
    if( VIEWS.hasOwnProperty("LOOP") && VIEWS.LOOP.valid &&
        VIEWS.hasOwnProperty("FMAX_II") && VIEWS.FMAX_II.valid &&
        VIEWS.hasOwnProperty("SCHEDULE") && VIEWS.SCHEDULE.valid &&
        isValidHierarchyTree() ) {
      treeJSON['nodes'].forEach(function(funcObj) {
        let orderedBlock = [];
        if (scheduleJSON.hasOwnProperty(funcObj['id']) && scheduleJSON[funcObj['id']].hasOwnProperty('nodes')) {
          scheduleJSON[funcObj['id']]['nodes'].forEach(function(blockObj) {
            let block = { 'name': blockObj['name'],
                          'id': blockObj['id'] };
            orderedBlock.push(block);
          });
        }

        let funcName = funcObj['name'];
        funcOrderedBlock[funcName] = {};
        funcOrderedBlock[funcName]['children'] = orderedBlock;
        funcOrderedBlock[funcName]['id'] = funcObj['id'];
      });
    }
    ...
    function addReportColumn(reportEnum) {
        ...
        switch (reportEnum) {
          ...
          case VIEWS.LOOP:
            ...
            return;
          case VIEWS.FMAX_II:
            report += "f<sub>MAX</sub> II Report (deprecated)";
            report += "</div><div class='card-body report-body' onscroll='adjustToWindowEvent()'>";
          ...
          case VIEWS.SCH_TABLE:
            report += "</div><div class='card-body report-body'>";
...
function goToView(viewId, update) {
    ...
    if (view != newView) {
      ...
      if (view == VIEWS.DOT && isValidDot) {
        ...
        if (!CTRLS.SOURCE.collapsed) collapseAceEditor();
        if (!CTRLS.DETAILS.collapsed) collapseDetails();
      }
    }
    if (update) {
        updateURLHash();
    ...
    switch (view) {
      ...
      case VIEWS.LOOP:
        view['tree'].draw();
        let loopInfo = new FPGALoop(document.getElementById("report-pane-view" + view.value));
        view['tree'].vTree.setClickCallback(loopInfo.draw);
        break;
      case VIEWS.LMEM:
...
// Use to toggle tree - use by collaspe tree button
...
function adjustReportPaneWidth() {
  // col width = 2, 5, 5
  let vNextWidth;
  if (!CTRLS.TREE.collapsed && !CTRLS.SOURCE.collapsed)     vNextWidth = "col-sm-5";
  else if (CTRLS.TREE.collapsed && !CTRLS.SOURCE.collapsed) vNextWidth = "col-sm-7";
  else if (!CTRLS.TREE.collapsed && CTRLS.SOURCE.collapsed) vNextWidth = "col-sm-10";
  else vNextWidth = "col-sm-12";
...
stickTableHeader()
{
    if (view !== VIEWS.AREA_SYS && view !== VIEWS.AREA_SRC && view !== VIEWS.LOOP && view !== VIEWS.VERIF && view !== VIEWS.INCREMENTAL && view != VIEWS.FMAX_II) return;
...
function setReportPaneHeight() {
    ...
    $('#tree-list, #report-pane, #editor-pane').css('height', parseInt(viewPortHeight - navBarHeight - detailHeight));
